...
if __name__=="__main__":
    ...
    for seas in SEASONS: 
        ...
        ds=[xr.open_dataset(file) for file in list_output_mdl[:-3]]

        ds=xr.concat(ds,["_".join(os.path.basename(file).split("_")[:3]) if "stations" not in file else\
                        "_".join(os.path.basename(file).split("_")[:2]) for file in list_output_mdl[:-3]]).\
                    rename({"concat_dim":"correction"})
        

        ds_tri=get_triveneto(ds,sta_val)
        ds_tri=ds#get_triveneto(ds,sta_val)
        
        ds_ori=ds_tri.sel(correction=~ds_tri.correction.str.contains("stations|EQM|QDM")).mean(dim='correction')
        ...
        bc_ori=EvaluatorBiasCorrection(ds_sta.pr,ds_ori.pr)
        bc_eqm=EvaluatorBiasCorrection(ds_sta.pr,ds_eqm.pr)
        bc_qdm=EvaluatorBiasCorrection(ds_sta.pr,ds_qdm.pr)
