...
if __name__=="__main__":
    ...
    #         figsize=(12,8),
    #         nrow=1,ncol=1,
    #         list_to_plot=[q_999_st.pr],
    #         name_fig=f"EVERYTHING",
    #         list_titles= ["STATIONS VALIDATION"],
    #         levels=[lvl_q],
    #         suptitle=f"",
    #         # name_metric=["[mm/h]","[mm/h]","[mm/h]"],
    #         name_metric=["Heavy Prec"],
    #         SET_EXTENT=True,
    #         cmap=[cmap_q],
    #         SAVE=False
    # )
    for seas in SEASONS: 
        if REF =="STATIONS":
            ...
            sphera  = [xr.open_mfdataset(f"{PATH_COMMON_DATA}/reanalysis/SPHERA/pr/*{yr}*") for yr in np.arange(2000,2010)]
            sta_val = xr.concat(sphera,dim='time') 
            sta_val['longitude']=mod.lon.values
            sta_val['latitude']=mod.lat.values
            sta_val=get_triveneto(sta_val.rename({'longitude':'lon','latitude':'lat'}),q_999_st.pr)
            sta_val=sta_val.isel(time=sta_val['time.season'].isin(seas))
        ...
        if BOOT_ORI:
            ...
            for mdl in list_mdl:
                ...
                if mdl == "ETH":
                    ...
                    ds_mod_ori.append(get_triveneto(mod_adjust.pr.isel(time=mod_adjust["time.season"].isin(seas)),sta_val))
                elif mdl == "ICTP":
                    if ADJUST=="TRAIN":
                    ...
                    ds_mod_ori.append(get_triveneto(xr.open_mfdataset(list_mod).pr.isel(time=xr.open_mfdataset(list_mod)["time.season"].isin(seas)),
                                                    sta_val).load())
                else:
                    if ADJUST=="TRAIN":
